...
import com.ants.order.pay.fuyou.entity.OmsOrderItem;
import com.ants.order.pay.fuyou.mapper.OmsOrderItemMapper;
...
public class OrderServerImpl implements IOrderServer {
    ...
    private void addOmsOrderItem(String goodValue, final OmsOrderDto omsOrderDto) {
        if (goodValue == null || "".equals(goodValue)) {
            throw new BusinessException(String.format("赠品子订单创建失败，参数goodValue: %s", goodValue));
        }
        if (omsOrderDto == null || omsOrderDto.getStoreId() == null) {
            throw new BusinessException(String.format("赠品子订单创建失败，参数omsOrderDto: %s", JSONObject.toJSONString(omsOrderDto)));
        }
        ...
        Arrays.stream(split).forEach(goodsId -> {
            ...
            if (!checkAdd) {
                throw new BusinessException(String.format("赠品子订单创建失败，参数omsOrderItem: %s", JSONObject.toJSONString(omsOrderItem)));
            }
        });
    }
}
